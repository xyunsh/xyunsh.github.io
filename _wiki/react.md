---
layout: wiki
title: React
categories: Web
description: 收集React开发资料。
keywords: React
---

- <https://reactjs.org/>
- <https://github.com/facebook/react>

### nextjs
- [中文文档](http://nextjs.frontendx.cn/)
- <https://nextjs.org/>

#### tips
- [Learning React](https://github.com/petehunt/react-howto/blob/master/README-zh.md)
- [Redux的全家桶与最佳实践](https://zhuanlan.zhihu.com/p/22405838)
- [常用的react components/](http://react-component.github.io/badgeboard/)

#### refresh and load more
- [The simplest way to make a pull-to-refresh and infinite tableview with ReactJS](https://medium.com/@CalvinChankf/the-simplest-way-to-make-a-pull-to-refresh-and-infinite-tableview-with-reactjs-18ecf85429ea#.sicdlhmpr)
- [Best Practices for Component State in React.js](http://brewhouse.io/blog/2015/03/24/best-practices-for-component-state-in-reactjs.html)
- [React Pull to Refresh](https://github.com/bryaneaton13/react-pull-to-refresh)
- [pullDownLoadMore](https://github.com/JenJieJu/react-pullDownLoadMore/blob/master/PageView.js)
- [reactjs chat app infinite scroll history using redux](https://www.pubnub.com/blog/2016-06-28-reactjs-chat-app-infinite-scroll-history-using-redux/)


#### Redux
- [React + Redux 项目工程架构实战](http://lifei.github.io/2016/01/22/react-redux-best-practice/)
- [SegmentFault 技术周刊 Vol.9 - 进阶吧！React](https://zhuanlan.zhihu.com/p/23041860)
- [Redux-saga 中文文档](https://github.com/superRaytin/redux-saga-in-chinese)


#### tools
- [immutable-js](https://facebook.github.io/immutable-js/)
- [normalizr](https://github.com/paularmstrong/normalizr)
- [gl-react](https://projectseptemberinc.gitbooks.io/gl-react/content/)


#### redux
- [redux saga](https://github.com/yelouafi/redux-saga)
- [Redux-saga 中文文档](http://leonshi.com/redux-saga-in-chinese/index.html)

### GraphQL
- [解读GraphQL(一): WHAT & WHY](https://zhuanlan.zhihu.com/p/26522359)
- [解读GraphQL(二): 使用Apollo Data构建GraphQL应用](https://zhuanlan.zhihu.com/p/26522388)
- [解读GraphQL(三)：Relay—面向未来的前端框架](https://zhuanlan.zhihu.com/p/26522410)
- [react-admin](https://github.com/marmelab/react-admin)

#### frameworks
- [relay](https://github.com/facebook/relay)
- [apollo graphQL](https://github.com/apollographql)
 * [[译]为什么选用 Apollo：它的优点、缺点和备选方案](https://zhuanlan.zhihu.com/p/39378504)
 * [A complete React with Apollo and GraphQL Tutorial](https://www.robinwieruch.de/react-graphql-apollo-tutorial/)
 * [learnapollo](https://github.com/learnapollo/learnapollo)

#### UI
 - [photonkit](http://photonkit.com/)
 - [reactdesktop](http://reactdesktop.js.org/)